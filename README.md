...
일정 알림(텍스트 + 음성 TTS) + 참가신청(RSVP), 주간 숙제 체크리스트, 주사위, 달력,
강화/돌파 시뮬레이터 + 기대비용 계산기·몬테카를로 시뮬·명예의 전당까지
**총 19개의 슬래시 명령어**를 제공합니다.
...
### 🔧 빌드 공유 (비활성화됨)
> 빌드 공유 기능(`/빌드등록` `/빌드검색` `/빌드목록` `/빌드삭제`)은 현재 **비활성화** 상태입니다.
> 코드([src/commands/build.js](src/commands/build.js))와 `builds` 테이블 데이터는 보존되어 있으며,
> `src/index.js`·`src/deploy-commands.js` 의 주석을 해제하고 `npm run deploy` 를 재실행하면 복원됩니다.
...
| 명령어 | 설명 |
|---|---|
| `/숙제` | 이번 주 내 숙제 체크리스트 (개인/ephemeral) + 버튼으로 체크·진행도 카운트 |
| `/숙제현황` | 이번 주 서버 멤버별 숙제 완료 현황 |
| `/숙제설정 목록` | 등록된 숙제 항목 조회 |
| `/숙제설정 추가 이름:<> [횟수:N]` | 숙제 항목 추가 |
| `/숙제설정 삭제 항목id:<>` | 숙제 항목 삭제 |
| `/숙제설정 수정 항목id:<> [이름:] [횟수:] [순서:]` | 숙제 항목 수정 |

> **초기화**: 매주 **수요일 10:00 KST**(점검 종료 시점 가정)에 모든 멤버의 체크리스트가 자동으로 새 주차로 넘어갑니다. cron 으로 데이터를 지우는 방식이 아니라, 진행도를 "주차"에 묶어 저장하고 현재 주차만 표시하는 방식이라 **봇이 그 시각에 꺼져 있어도 정확**하고 지난 주 기록도 보존됩니다. (초기화 요일/시각은 [src/utils/dateUtils.js](src/utils/dateUtils.js) 의 `RESET_WDAY`/`RESET_HOUR` 상수로 조정)
> **기본 항목**: 길드에서 `/숙제` 를 처음 쓰면 시공공격·시공방어·전장(3회)·은공물질변환 4종이 자동 등록되며, 누구나 `/숙제설정` 으로 자유롭게 편집할 수 있습니다.
> **횟수형 숙제**: 전장처럼 여러 번 하는 숙제는 버튼을 누를 때마다 1/3 → 2/3 → 3/3 으로 진행도가 차고, 다 채우면 ✅ 로 표시됩니다.
...
└── src/
    ...
    ├── commands/
    │   ├── build.js            # /빌드등록 /빌드검색 /빌드목록 /빌드삭제 (비활성화)
    ...
    │   └── homework.js         # /숙제 /숙제현황 /숙제설정 + 숙제 체크 버튼 핸들러
    ├── data/
...
6개 테이블 — `builds`, `schedules`, `inventory`, `schedule_rsvp`, `homework_tasks`, `homework_progress`. 모든 테이블이 `guild_id` 로 서버 단위 격리.
자세한 컬럼·관계는 [schema.png](schema.png) 참고.

| 테이블 | 용도 | 격리 단위 |
|---|---|---|
| `builds` | 빌드 공유 (기능 비활성화, 데이터 보존) | 서버별 |
| `schedules` | 레기온 일정 + 자동 알림 | 서버별 |
| `inventory` | 강화/돌파 시뮬레이터 장비 | 서버 × 사용자별 |
| `schedule_rsvp` | 일정 참가신청(참가/불참/보류) | 일정 × 사용자별 |
| `homework_tasks` | 주간 숙제 항목 정의 | 서버별 |
| `homework_progress` | 주간 숙제 진행도 | 서버 × 사용자 × 주차별 |

> 성능 인덱스: `inventory(guild_id,user_id)`, `schedules(scheduled_at)`, `builds(guild_id,created_at)`, `schedule_rsvp(guild_id,user_id)`, `homework_tasks(guild_id,active,sort_order)`, `homework_progress(guild_id,week_key,user_id)` — 데이터 증가 시 풀스캔을 방지. 봇 기동 시 멱등 마이그레이션으로 자동 생성.
