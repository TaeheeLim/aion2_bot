...
빌드 공유, 일정 알림(텍스트 + 음성 TTS), 주사위, 달력, 강화/돌파 시뮬레이터까지
**총 16개의 슬래시 명령어**를 제공합니다.
...
| 명령어 | 설명 |
|---|---|
| `/일정등록` | 레기온 일정 등록 + 60/30/10/3분 전 자동 알림 + 음성채널 TTS 옵션 |
...
| 명령어 | 설명 |
|---|---|
| `/장비생성 종류:<영웅\|전념의룬> 이름:...` | 강화 1강 / 돌파 0 상태로 새 장비 생성 |
| `/장비목록` | 내 인벤토리(최대 30개) + 사용자 전체 누적 사용량 (ephemeral) |
...
└── src/
    ...
    ├── data/
    │   └── enhanceData.js      # 강화/돌파 확률·비용 테이블 (엑셀 추출)
    ├── services/
...
3개 테이블 — `builds`, `schedules`, `inventory`. 모든 테이블이 `guild_id` 로 서버 단위 격리.
